...
import WeftWastageList from '../weftwastage/WeftWastageList';


const WeftWastageInfo = () => {
  ...
  useLayoutEffect(() => {
    navigation.setOptions({
      headerTitle: () => (
        <Text style={{ color: colors.textLight, fontWeight: 'bold', fontSize: 16 }}>Weft Wastage</Text>
      ),
  ...
  const handleSubmit = async() => {
    ...
    if (Object.keys(newErrors).length === 0) {
      ...
      if (output){
        ...
        setLoading(true);
        const response = await postToAPI('/insert_weft_wastage', data);
        setLoading(false);
        if (response.rval > 0){
          Toast.show({
            ...toastConfig.success,
  ...
  return (
    <PaperProvider>
      <ScrollView style={styles.scrollContainer}>
        ...
        <View>
          <WeftWastageList getItemDescription={getItemDescription} qrData={qrData} errors={errors} checkInput={checkInput} ProductionLocation={getProductionLocation} navigateToCamera={navigateToCamera}/>
        </View>
...
export default WeftWastageInfo;
